...
const dbPath = path.join(dataDir, 'aipm.json');
  ...
  try {
    if (fs.existsSync(dbPath)) {
      ...
      store = {
        ...
        knowledge_base: loadedData.knowledge_base || [],
        knowledge_projects: loadedData.knowledge_projects || [],
        user_skills: loadedData.user_skills || [],
        permissions: loadedData.permissions || [],
        role_permissions: loadedData.role_permissions || [],
        user_permissions: loadedData.user_permissions || [],
        project_stages: loadedData.project_stages || [],
        workflow_templates: loadedData.workflow_templates || [],
      };
    }
  } catch (e) {
  ...
  knowledgeBase: {
    ...
    delete: (id: string) => {
      ...
      if (idx >= 0) {
        ...
        store.knowledge_projects = store.knowledge_projects.filter(kp => kp.knowledge_id !== id);
        saveStore();
        return true;
      }
      return false;
    ...
    search: (query: string) => {
      ...
      return store.knowledge_base.filter(k => 
        k.title.toLowerCase().includes(lowerQuery) || 
        k.content.toLowerCase().includes(lowerQuery)
      );
    },
    findByCategory: (category: string) => store.knowledge_base.filter(k => k.category === category),
    findByTag: (tag: string) => store.knowledge_base.filter(k => k.tags && k.tags.includes(tag)),
    getCategories: () => [...new Set(store.knowledge_base.map(k => k.category).filter(c => c))],
